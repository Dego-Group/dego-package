...
export type DegoFunctionComponent = (
...
} & ({ rawHtml: RawHtml; isNode: false } | { stringHtml: string; isNode: true })

export type RawHtml = Element
export type DegoId = {
  isDegoId: true
  id: string
  interactions: { id: string; callback: (e: any) => void }[]
  storeReferences: { id: string }[]
  hooks: {
...
export type DegoComponentInvocation<
  P = Record<string, any> | undefined,
  PL = Record<string, any> | undefined
> = {
    ...
    degoRenderProgress: {
      ...
      pendingReferenceIds: Set<
        { id: string } & (
          ...
          | { type: 'store'; data: StoreData<Element | undefined> }
        )
      >
    }
